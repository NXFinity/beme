...
export class TopNavComponent implements OnInit {
  ...
  ngOnInit() {
    this.appMenuService.getAppMenus().subscribe(menus => {
      this.menus = menus;
    });
  }
  ...
  handleTopNavControlAction(event: MouseEvent, direction: string) {
    ...
    const containerWidth =
      (document.querySelector('.app-top-nav') as HTMLElement).clientWidth -
      (document.querySelector('.app-top-nav') as HTMLElement).clientHeight * 2;
    let totalWidth = 0;
